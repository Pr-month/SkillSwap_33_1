...
// node_modules/.pnpm/tsup@8.5.0_typescript@5.1.3/node_modules/tsup/assets/cjs_shims.js
...
function checkPublicAsset(source, publicDir) {
  if (!source.startsWith("/") || !publicDir)
    return false;
  const publicFile = import_node_path.default.posix.join(publicDir, source);
  if (publicFileCache.has(publicFile))
    return publicFileCache.get(publicFile);
  const exists = import_node_fs.default.existsSync(publicFile);
  if (exists)
...
async function rebaseStyleUrls(moduleId, result, config) {
  ...
  if (sourceMap.sources.length < 2)
    return result;
  const moduleDir = import_node_path2.default.dirname(moduleId);
  ...
  const replacements = await Promise.all(
    sources.map(async (source2, index) => {
      ...
      if (source2.indexOf(config.root.slice(1)) === 0)
        source2 = `/${source2}`;
      const filepath = import_node_path2.default.isAbsolute(source2) ? source2 : import_node_path2.default.resolve(moduleDir, source2);
      const fileDir = import_node_path2.default.dirname(filepath);
      ...
      return assets.map((asset) => {
        const assetPath = import_node_path2.default.resolve(fileDir, asset);
        const replacement = import_node_path2.default.relative(moduleDir, assetPath);
...
function VitePluginLibAssets(options = {}) {
  ...
  const jsTypeFilter = (0, import_vite3.createFilter)(JS_TYPES_RE);
  ...
  const emitFile = (context, id, content) => {
    ...
    assetPath = typeof outputDir === "function" ? import_node_path3.default.posix.join(outputDir(url, pureId, resourceQuery), url) : import_node_path3.default.posix.join(outputDir, url);
    const filename = assetPath.replace(resourceQuery, "");
    const fullname = import_node_path3.default.join(import_node_path3.default.isAbsolute(outDir) ? process.cwd() : "", outDir, assetPath);
    const emitted = {
      fileName: filename,
      name: fullname,
  ...
  const extractFromFile = async (context, id) => {
    ...
    if (import_node_path3.default.extname(pureId) === ".vue") {
  ...
  return {
    ...
    async resolveId(source, importer = "", opts) {
      ...
      if (opts.custom?.[pluginName]?.fromResolveId)
        return null;
      let id;
      if (import_node_path3.default.isAbsolute(source)) {
        id = source;
      } else if (source.startsWith(".")) {
        id = import_node_path3.default.resolve(import_node_path3.default.dirname(importer), source);
      } else {
      ...
      if (jsTypeFilter(id) || cssLangFilter(id)) {
        ...
        const validAssets = assetsExtracted.map((aid) => import_node_path3.default.resolve(import_node_path3.default.dirname(id), aid)).filter((id2) => filter(id2)).map((id2) => ({ id: id2, content: getAssetContent(id2) })).filter(({ content }) => limit && content ? content.byteLength > limit : true);
    ...
    async writeBundle(_, outputBundle) {
      ...
      const bundleSourceMap = Object.keys(outputBundle).reduce((map, name2) => {
        const filePath = import_node_path3.default.posix.join(outputDir, name2);
      ...
      Object.keys(bundleSourceMap).filter((name2) => bundleSourceMap[name2] !== processedSourceMap[name2]).forEach((name2) => {
        const outputPath2 = import_node_path3.default.posix.join(outputDir, name2);
