...
let UsersService = class UsersService {
    ...
    async findUserById(userId) {
        ...
        if (!user) {
            throw new common_1.NotFoundException(`Пользователь с ID ${userId} не существует`);
        }
        return user;
    }
    ...
    async update(id, updateUserDto) {
        const user = await this.findUserById(id);
        const patch = Object.fromEntries(Object.entries(updateUserDto).filter(([, v]) => v !== undefined));
        if (patch.city) {
    ...
    async remove(id) {
        ...
        if (!result.affected) {
            throw new common_1.NotFoundException(`Пользователь с ID ${id} не существует`);
        }
        return { message: 'Пользователь успешно удалён' };
    }
    async refresh(userId, newRefreshToken) {
    ...
    async findByEmail(email) {
        ...
        if (!user) {
            throw new common_1.NotFoundException(`Пользователь с email ${email} не существует`);
        }
        return user;
    }
